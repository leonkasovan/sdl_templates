#include <SDL.h>
#define FONT_HEIGHT 10
#define MAX_TEXT_LENGTH 1024
#define xx 10

static const unsigned char embedded_font[] = {};

void draw_char(SDL_Surface* surface, unsigned char symbol, int x, int y, unsigned short color) {
	x += (8 - 1) * 1;
	int flip = 0;
	if(symbol > 127) { // flip symbols over 128
		flip = 1;
		symbol -= 128;
	}
	const unsigned char* ptr = embedded_font + symbol * 8;

	for(int i = 0, ys = 0; i < 6; i++, ptr++, ys += 1)
		for(int col = 8 - 6, xs = x - col; col < 8; col++, xs -= 1)
			if((*ptr & 1 << col) && y + ys < surface->h && xs < surface->w )
				((unsigned short*)surface->pixels)[(y + flip * 4 + (1 - 2 * flip) * ys) * (surface->pitch >> 1) + xs] = color;

}

void draw_string(SDL_Surface* surface, const char* text, int orig_x, int orig_y, unsigned short color) {
	int x = orig_x, y = orig_y;
	while(*text) {
		if(*text == '\n') {
			x = orig_x;
			y += 8;
		} else {
			draw_char(surface, *text, x, y, color);
			x += 6;
		}
		text++;
	}
}

int main(int argc, char* argv[]) {
	int yy = 0;
	SDL_version compiled;
	SDL_VERSION(&compiled);
	char msg_global[MAX_TEXT_LENGTH];
	printf("We compiled against SDL version %u.%u.%u\n", compiled.major, compiled.minor, compiled.patch);

#ifdef SDL_2
	SDL_version linked;
	SDL_GetVersion(&linked);
	printf("But we are linking against SDL version %u.%u.%u\n", linked.major, linked.minor, linked.patch);

	puts("\nAvailable video drivers:");
	int numVideoDrivers = SDL_GetNumVideoDrivers();
	for (int i = 0; i < numVideoDrivers; i++) {
		puts(SDL_GetVideoDriver(i));
	}

	puts("\nAvailable audio drivers:");
	int numAudioDrivers = SDL_GetNumAudioDrivers();
	for (int i = 0; i < numAudioDrivers; i++) {
		puts(SDL_GetAudioDriver(i));
	}

	if (SDL_Init(SDL_INIT_JOYSTICK|SDL_INIT_VIDEO) < 0) {
		// Initialization failed
		fprintf(stderr, "SDL_Init failed: %s\n", SDL_GetError());
		return 1;
	}
	printf("Currently using audio driver: %s\n", SDL_GetCurrentAudioDriver());
	printf("Currently using video driver: %s\n", SDL_GetCurrentVideoDriver());

	SDL_DisplayMode current;
	SDL_GetCurrentDisplayMode(0, &current);
	SDL_Window *Screen = SDL_CreateWindow("SDL Info", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, current.w, current.h, SDL_WINDOW_FULLSCREEN);
    if (!Screen) {
		fprintf(stderr, "SDL_CreateWindow failed: %s\n", SDL_GetError());
		return 1;
	}
	//Get window surface
    SDL_Surface* screen = SDL_GetWindowSurface(Screen);
#else
	SDL_Surface* screen = SDL_SetVideoMode(640, 480, 16, SDL_FULLSCREEN|SDL_HWSURFACE);
#endif
	if (!screen) {
		fprintf(stderr, "SDL video mode set failed: %s\n", SDL_GetError());
		return 1;
	}

	// Create a back buffer 300 x 150
	SDL_Surface* backbuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 300, 150, 16, 0, 0, 0, 0);
	if (!backbuffer) {
		fprintf(stderr, "SDL_CreateRGBSurface failed: %s\n", SDL_GetError());
		return 1;
	}
	SDL_FillRect(backbuffer, NULL, SDL_MapRGB(backbuffer->format, 0, 0, 0)); // Clear the backbuffer with black.
	snprintf(msg_global, MAX_TEXT_LENGTH, "We compiled against SDL version %u.%u.%u", compiled.major, compiled.minor, compiled.patch);
#ifdef SDL_2	
	draw_string(backbuffer, msg_global, xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 255)); yy += FONT_HEIGHT;
	snprintf(msg_global, MAX_TEXT_LENGTH, "But we are linking against SDL version %u.%u.%u", linked.major, linked.minor, linked.patch);
	draw_string(backbuffer, msg_global, xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 255)); yy += FONT_HEIGHT * 2;
	snprintf(msg_global, MAX_TEXT_LENGTH, "Available video drivers: (using=%s)", SDL_GetCurrentVideoDriver());
	draw_string(backbuffer, msg_global, xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 255)); yy += FONT_HEIGHT;
	for (int i = 0; i < numVideoDrivers; i++) {
		draw_string(backbuffer, SDL_GetVideoDriver(i), xx, yy, SDL_MapRGB(backbuffer->format, 0, 255, 0)); yy += FONT_HEIGHT;
	}
	yy += FONT_HEIGHT;
	snprintf(msg_global, MAX_TEXT_LENGTH, "Available audio drivers: (using=%s)", SDL_GetCurrentAudioDriver());
	draw_string(backbuffer, msg_global, xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 255)); yy += FONT_HEIGHT;
	for (int i = 0; i < numAudioDrivers; i++) {
		draw_string(backbuffer, SDL_GetAudioDriver(i), xx, yy, SDL_MapRGB(backbuffer->format, 0, 255, 0)); yy += FONT_HEIGHT;
	}
	yy += FONT_HEIGHT;
#endif

	draw_string(backbuffer, "Hello world SDL in full screen 640x480", xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 255)); yy += FONT_HEIGHT;
	draw_string(backbuffer, "This is embedded font in GREEN", xx, yy, SDL_MapRGB(backbuffer->format, 0, 255, 0)); yy += FONT_HEIGHT;
	draw_string(backbuffer, "This is embedded font in BLUE", xx, yy, SDL_MapRGB(backbuffer->format, 0, 0, 255)); yy += FONT_HEIGHT;
	draw_string(backbuffer, "Press ESC or ANY button to quit", xx, yy, SDL_MapRGB(backbuffer->format, 255, 255, 0)); yy += FONT_HEIGHT;

	// Main loop
	int quit = 0;
	while (!quit) {
		SDL_Event ev;

		while (SDL_PollEvent(&ev)) {
			switch (ev.type) {
				case SDL_QUIT:
					quit = 1;
					break;
				case SDL_KEYDOWN:
					if (ev.key.keysym.sym == SDLK_ESCAPE)
						quit = 1;
					break;
				default:
					break;
			}
		}

		SDL_BlitSurface(backbuffer, NULL, screen, NULL);
#ifdef SDL_2
		SDL_UpdateWindowSurface(Screen); //Update the surface
#else
		SDL_Flip(screen);
#endif
	}

	// Clean up
#ifdef SDL_2
    SDL_DestroyWindow(Screen);
#endif	
	SDL_FreeSurface(backbuffer);
	SDL_Quit();

	return 0;
}